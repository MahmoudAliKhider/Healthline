...
exports.getUpcoming = async (req, res, next) => {
  try {
    ...
    if (!upcoming) return res.status(404).send("User not found !");
    ...
    if (!req.params.medicineId) {
      ...
      if (req.query.state == "Missed") req.query.state = "Completed";
      else req.query.state = "Missed";
  ...
  try {
    ...
    const promises = caregiver.circles.map((circle) => {
      return Upcomings.find({
        ...
        .populate("user", "_id image audio fullname")
        .populate("medicines.medicine", "_id name image audio time");
    });
...
exports.changeState = async (req, res, next) => {
  try {
    ...
    let upcoming = await Upcomings.findOne({
      user: req.params.patientId,
      createdAt: new Date().toISOString().split("T")[0],
    });
    if (!upcoming) return res.status(404).send("User not found !");
