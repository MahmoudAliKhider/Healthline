  ...
  try {
    ...
    if (req.query.user != "admin") {
      users = await User.find({
        registAs: req.query.user,
        isAdmin: false,
      })
        .select(
          "image fullname email phone dateOfBirth gender medicines circles"
        )
        .populate("medicines")
        .populate(
          "circles.id",
          "_id image fullname email phone dateOfBirth gender"
        );
...
exports.updateAdmin = async (req, res, next) => {
  try {
    ...
    if (req.query.state === "true") req.query.state = true;
    else req.query.state = false;

    if (user.isAdmin != req.query.state) {
      await User.findByIdAndUpdate(
        req.params.userId,
        { isAdmin: req.query.state },
        { new: true }
      );
      let message;
