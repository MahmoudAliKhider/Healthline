...
exports.postMedicines = async (req, res, next) => {
  try {
    ...
    const uploadedAudio = await imageHandler.UploadImage(
...
exports.getAllUserMedicines = async (req, res, next) => {
  try {
    const user = await User.findById(req.params.patientId).populate(
      "medicines"
    );
...
exports.updateMedicines = async (req, res, next) => {
  try {
    ...
    const updatedMedicines = await Medicine.findByIdAndUpdate(
      req.params.id,
      req.body,
      { new: true }
    );
...
exports.afterChanges = async (req, res, next) => {
  try {
    const isThere = await Upcomings.find({
